...
contract CurveStrategyForLPV1 is YakStrategy {
    ...
    function assignZapFunction() internal {
        if (zapSettings.poolType == PoolType.AAVE) {
            require(
                zapSettings.zapToken ==
                    ICurveStableSwapAave(zapSettings.zapContract).underlying_coins(
                        zapSettings.zapTokenIndex
                    ),
                "Wrong zap token index"
            );
            _zapToDepositToken = _zapToAaveLP;
        } else if (zapSettings.poolType == PoolType.CRYPTO) {
            require(
                zapSettings.zapToken ==
