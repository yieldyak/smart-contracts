...
contract BlizzStrategyV1 is YakStrategyV2 {
    ...
    function _deposit(address account, uint256 amount) private onlyAllowedDeposits {
        ...
        if (MAX_TOKENS_TO_DEPOSIT_WITHOUT_REINVEST > 0) {
            (uint256 poolTokenAmount, , uint256 estimatedTotalReward) = _checkReward();
            if (estimatedTotalReward > MAX_TOKENS_TO_DEPOSIT_WITHOUT_REINVEST) {
                _reinvest(poolTokenAmount, estimatedTotalReward);
            }
        }
     ...
     */
    function _reinvest(uint256 poolTokenAmount, uint256 estimatedTotalReward) private {
        ...
        uint256 depositTokenAmount = DexLibrary.swap(
            estimatedTotalReward.sub(devFee).sub(reinvestFee),
            address(rewardToken),
            address(depositToken),
