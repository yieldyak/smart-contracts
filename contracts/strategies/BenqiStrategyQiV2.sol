...
contract BenqiStrategyQiV2 is YakStrategyV2, ExponentialNoError {
    ...
    function _deposit(address account, uint256 amount) private onlyAllowedDeposits {
        ...
        if (MAX_TOKENS_TO_DEPOSIT_WITHOUT_REINVEST > 0) {
            (uint256 qiRewards, uint256 avaxRewards, uint256 totalQiRewards) = _checkRewards();
    ...
    function _reinvest(
        ...
        if (avaxBalance > 0) {
            ...
            DexLibrary.swap(avaxBalance, address(rewardToken1), address(rewardToken0), swapPairToken1);
        }
