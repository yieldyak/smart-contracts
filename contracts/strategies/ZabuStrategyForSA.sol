...
/**
 * @notice Strategy for Zabu single asset
 */
contract ZabuStrategyForSA is YakStrategy {
  using SafeMath for uint;

  IZabuChef public stakingContract;
  IPair private swapPairToken;
  IPair private swapPairWAVAXZabu;
  ...
  constructor(
    ...
    swapPairWAVAXZabu = IPair(_swapPairWAVAXZabu);
  ...
  function _deposit(address account, uint amount) internal {
    require(DEPOSITS_ENABLED == true, "ZabuStrategyForSA::_deposit");
  ...
  function _withdrawDepositTokens(uint amount) private {
    require(amount > 0, "ZabuStrategyForSA::_withdrawDepositTokens");
    stakingContract.withdraw(PID, amount);
  }

  function reinvest() external override onlyEOA {
    uint unclaimedRewards = checkReward();
    require(unclaimedRewards >= MIN_TOKENS_TO_REINVEST, "ZabuStrategyForSA::reinvest");
    _reinvest(unclaimedRewards);
  }
  ...
  function _reinvest(uint amount) private {
    ...
    if (address(swapPairWAVAXZabu) != address(0)) {
      if (address(swapPairToken) != address(0)) {
        uint amountWavax = DexLibrary.swap(depositTokenAmount, address(rewardToken), address(WAVAX), swapPairWAVAXZabu);
        depositTokenAmount = DexLibrary.swap(amountWavax, address(WAVAX), address(depositToken), swapPairToken);
      }
      else {
        depositTokenAmount = DexLibrary.swap(depositTokenAmount, address(rewardToken), address(WAVAX), swapPairWAVAXZabu);
      }
    }
  ...
  function _stakeDepositTokens(uint amount) private {
    require(amount > 0, "ZabuStrategyForSA::_stakeDepositTokens");
  ...
  function rescueDeployedFunds(uint minReturnAmountAccepted, bool disableDeposits) external override onlyOwner {
    ...
    require(balanceAfter.sub(balanceBefore) >= minReturnAmountAccepted, "ZabuStrategyForSA::rescueDeployedFunds");
