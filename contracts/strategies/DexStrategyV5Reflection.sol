...
contract DexStrategyV5Reflection is YakStrategy {
    ...
    function _convertRewardTokensToDepositTokens(uint256 amount) private returns (uint256) {
        ...
        uint256 amountIn0 = amount.sub(token0BalanceFactor).add(token1BalanceFactor).add(burnBalance).div(2);
        uint256 amountIn1 = amount.sub(token1BalanceFactor).add(token0BalanceFactor).sub(burnBalance).div(2);
