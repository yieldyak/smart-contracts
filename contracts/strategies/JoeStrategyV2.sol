...
import "../lib/DexInteractionHandler.sol";
...
contract JoeStrategyV2 is YakStrategy {
    ...
    function assignSwapPairSafely(address _swapPairWAVAXJoe, address _extraTokenSwapPair, address _swapPairToken0, address _swapPairToken1) private {
        require(
            DexInteractionHandler.checkSwapPairCompatibility(IPair(_swapPairWAVAXJoe), address(WAVAX), address(poolRewardToken)),
            "_swapPairWAVAXJoe is not a WAVAX-Joe pair"
        );
        require(
            _swapPairToken0 == address(0)
            || DexInteractionHandler.checkSwapPairCompatibility(IPair(_swapPairToken0), address(WAVAX), IPair(address(depositToken)).token0()),
            "_swapPairToken0 is not a WAVAX+deposit token0"
        );
        require(
            _swapPairToken1 == address(0)
            || DexInteractionHandler.checkSwapPairCompatibility(IPair(_swapPairToken1), address(WAVAX), IPair(address(depositToken)).token1()),
        ...
        require(
            ...
            || DexInteractionHandler.checkSwapPairCompatibility(IPair(_extraTokenSwapPair), address(WAVAX), extraRewardToken),
    ...
    function _convertRewardIntoWAVAX(uint pendingJoe, address extraRewardToken, uint pendingExtraReward) private returns (uint) {
        ...
        if (extraRewardToken == address(poolRewardToken)) {
            convertedAmountWAVAX = DexInteractionHandler.swap(
        ...
        if (
            ...
            && DexInteractionHandler.checkSwapPairCompatibility(swapPairExtraToken, extraRewardToken, address(WAVAX))
        ) {
            convertedAmountWAVAX = convertedAmountWAVAX.add(
                DexInteractionHandler.swap(pendingExtraReward, extraRewardToken, address(WAVAX), swapPairExtraToken)
            );
        }
        return convertedAmountWAVAX;
    }
    ...
    function _reinvest(uint _pendingJoe, address _extraRewardToken, uint _pendingExtraToken, uint _pendingWavax) private {
        ...
        (uint amountOutToken0, uint amountOutToken1) = DexInteractionHandler.convertWAVAXToDepositTokens(
            amount.sub(devFee).sub(adminFee).sub(reinvestFee), address(depositToken), swapPairToken0, swapPairToken1
        );

        uint depositTokenAmount = DexInteractionHandler.addLiquidity(address(depositToken), amountOutToken0, amountOutToken1);
    ...
    function setExtraRewardSwapPair(address swapPair) external onlyDev {
        ...
        ( ,address extraRewardToken, , ) = stakingContract.pendingTokens(PID, address(this));
        require(
            DexInteractionHandler.checkSwapPairCompatibility(IPair(swapPair), address(WAVAX), extraRewardToken),
            "_swapPairWAVAXJoe is not a WAVAX-extra reward pair, check stakingContract.pendingTokens"
        );
    ...
    function checkReward() public override view returns (uint) {
        (uint poolTokenAmount, address extraRewardTokenAddress, uint extraRewardTokenAmount, uint rewardTokenAmount) = _checkReward();
        uint estimatedWAVAX = DexInteractionHandler.estimateConversionThroughPair(
            poolTokenAmount,
            address(poolRewardToken), address(WAVAX),
            swapPairWAVAXJoe
        );
        if (
            ...
            && DexInteractionHandler.checkSwapPairCompatibility(swapPairExtraToken, extraRewardTokenAddress, address(WAVAX))
        ) {
            estimatedWAVAX.add(
                DexInteractionHandler.estimateConversionThroughPair(
