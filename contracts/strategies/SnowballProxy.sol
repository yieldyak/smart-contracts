...
contract SnowballProxy {
    ...
    function withdraw(
        ...
        snowballVoter.safeExecute(_token, 0, abi.encodeWithSignature("transfer(address,uint256)", msg.sender, _balance));
        return _balance;
    }

    function withdrawAll(address _stakingContract, address _snowGlobe, address _token) external onlyStrategy(_stakingContract) {
