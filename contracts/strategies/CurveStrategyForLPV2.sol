...
contract CurveStrategyForLPV2 is YakStrategy {
    ...
    function(uint256, address, address, CurveSwap.Settings memory)
        internal
        returns (uint256) _zapToDepositToken;
    ...
    function removeReward(address rewardToken) public onlyDev {
        ...
        require(
            rewards[index].reward == rewardToken,
            "CurveStrategyForLPV2::Reward not found!"
        );
    ...
    function _estimateRewardConvertedToAvax(uint256[] memory amounts)
        private
        view
        returns (uint256)
    {
    ...
    function _safeTransfer(
        ...
        require(
            ...
            "CurveStrategyForLPV2::TRANSFER_FROM_FAILED"
        );
    }
    ...
    function _calculateRewards(address _rewardToken) public view returns (uint256) {
        ...
        uint256 lastRewardUpdateTime = ICurveRewardsClaimer(rewardContractAddress)
            .last_update_time();
        ...
        if (integralFor < integral) {
            strategyUnclaimed = stakingContract
                .balanceOf(address(this))
                .mul(integral.sub(integralFor))
                .div(10e18);
        }
