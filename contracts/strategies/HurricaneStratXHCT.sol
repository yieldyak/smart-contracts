...
contract CompoundingHct is YakStrategy {
  ...
  function withdraw(uint amount) external override {
    ...
    if (depositTokenAmount > 0) {
      ...
      require(depositToken.transfer(msg.sender, depositTokenAmount), "CompoundingHct::withdraw");
  ...
  function _withdrawDepositTokens(uint amount) private {
    ...
    uint xHctAmount = _getxHctForHct(amount);
  ...
  function _reinvest(uint amount) private {
    ...
    if (devFee > 0) {
      require(rewardToken.transfer(devAddr, devFee), "CompoundingHct::_reinvest, dev");
    }

    uint adminFee = amount.mul(ADMIN_FEE_BIPS).div(BIPS_DIVISOR);
    if (adminFee > 0) {
      require(rewardToken.transfer(owner(), adminFee), "CompoundingHct::_reinvest, admin");
    }

    uint reinvestFee = amount.mul(REINVEST_REWARD_BIPS).div(BIPS_DIVISOR);
    if (reinvestFee > 0) {
      require(rewardToken.transfer(msg.sender, reinvestFee), "CompoundingHct::_reinvest, reward");
    }
   ...
   */
  function _convertHctToxHct(uint amount) private {
   ...
   * @param amount xHct
   */
  function _stakexHct(uint amount) private {
   ...
   * @return xHct shares
   */
  function _getxHctForHct(uint amount) private view returns (uint) {
    uint HctBalance = depositToken.balanceOf(address(conversionContract));
    uint xHctShares = xHct.totalSupply();
   ...
   * @return Hct tokens
   */
  function _getHctForxHct(uint amount) private view returns (uint) {
    uint HctBalance = depositToken.balanceOf(address(conversionContract));
    uint xHctShares = xHct.totalSupply();
    if (HctBalance.mul(xHctShares) == 0) {
      return amount;
  ...
  function rescueDeployedFunds(uint minReturnAmountAccepted, bool disableDeposits) external override onlyOwner {
    ...
    require(balanceAfter.sub(balanceBefore) >= minReturnAmountAccepted, "CompoundingHct::rescueDeployedFunds");
