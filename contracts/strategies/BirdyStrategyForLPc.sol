...
import "../interfaces/IBirdyChef.sol";
 ...
 */
contract BirdyStrategyForLPc is YakStrategy {
    using SafeMath for uint256;

    IRouter public router;
    IBirdyChef public stakingContract;
    ...
    constructor(
        ...
        stakingContract = IBirdyChef(_stakingContract);
    ...
    function _deposit(address account, uint256 amount) internal {
        require(DEPOSITS_ENABLED == true, "BirdyStrategyForLP::_deposit");
    ...
    function withdraw(uint256 amount) external override {
        ...
        if (depositTokenAmount > 0) {
            ...
            require(
                depositToken.transfer(msg.sender, depositTokenAmount.sub(withdrawFee)),
                "BirdyStrategyForLP::withdraw"
            );
    ...
    function _withdrawDepositTokens(uint256 amount) private {
        require(amount > 0, "BirdyStrategyForLP::_withdrawDepositTokens");
        stakingContract.withdraw(PID, amount);
    }

    function reinvest() external override onlyEOA {
        uint256 unclaimedRewards = checkReward();
        require(unclaimedRewards >= MIN_TOKENS_TO_REINVEST, "BirdyStrategyForLP::reinvest");
        _reinvest(unclaimedRewards);
    }
    ...
    function _reinvest(uint256 amount) private {
        ...
        if (adminFee > 0) {
            require(rewardToken.transfer(owner(), adminFee), "BirdyStrategyForLP::_reinvest, admin");
        }

        uint256 reinvestFee = amount.mul(REINVEST_REWARD_BIPS).div(BIPS_DIVISOR);
        if (reinvestFee > 0) {
            require(rewardToken.transfer(msg.sender, reinvestFee), "BirdyStrategyForLP::_reinvest, reward");
        }
    ...
    function _stakeDepositTokens(uint256 amount) private {
        require(amount > 0, "BirdyStrategyForLP::_stakeDepositTokens");
        stakingContract.deposit(PID, amount);
    }

    function checkReward() public view override returns (uint256) {
        uint256 pendingReward = stakingContract.pendingBIRD(PID, address(this));
    ...
    function _convertRewardTokensToDepositTokens(uint256 amount) private returns (uint256) {
        ...
        require(amountIn > 0, "BirdyStrategyForLP::_convertRewardTokensToDepositTokens");

        // swap to token0
        uint256 path0Length = 3;
    ...
    function rescueDeployedFunds(uint256 minReturnAmountAccepted, bool disableDeposits) external override onlyOwner {
        ...
        require(balanceAfter.sub(balanceBefore) >= minReturnAmountAccepted, "BirdyStrategyForLP::rescueDeployedFunds");
