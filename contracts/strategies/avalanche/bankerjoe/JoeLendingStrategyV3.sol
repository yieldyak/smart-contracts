...
contract JoeLendingStrategyV3 is YakStrategyV2 {
    ...
    function _deposit(address account, uint256 amount) private onlyAllowedDeposits {
        ...
        require(depositToken.transferFrom(msg.sender, address(this), amount), "JoeLendingStrategyV3::transfer failed");
    ...
    function withdraw(uint256 amount) external override {
        ...
        require(depositTokenAmount >= minMinting, "JoeLendingStrategyV3:: below minimum withdraw");
    ...
    function _withdrawDepositTokens(uint256 amount) private {
        _unrollDebt(amount);
        require(tokenDelegator.redeemUnderlying(amount) == 0, "JoeLendingStrategyV3::redeem failed");
        uint256 balance = tokenDelegator.balanceOfUnderlying(address(this));
        uint256 borrow = tokenDelegator.borrowBalanceCurrent(address(this));
        if (balance > 0) {
    ...
    function reinvest() external override onlyEOA {
        (uint256 joeRewards, uint256 avaxBalance, uint256 amountToReinvest) = _checkRewards();
        require(amountToReinvest >= MIN_TOKENS_TO_REINVEST, "JoeLendingStrategyV3::reinvest");
        _reinvest(joeRewards, avaxBalance, amountToReinvest);
        claimAVAXRewards();
    }
    ...
    function _rollupDebt(uint256 principal, uint256 borrowed) internal {
        ...
        while (supplied < lendTarget) {
            ...
            require(tokenDelegator.borrow(toBorrowAmount) == 0, "JoeLendingStrategyV3::borrowing failed");
            require(tokenDelegator.mint(toBorrowAmount) == 0, "JoeLendingStrategyV3::lending failed");
    ...
    function _unrollDebt(uint256 amountToFreeUp) internal {
        ...
        while (toRepay > 0) {
            ...
            require(tokenDelegator.redeemUnderlying(unrollAmount) == 0, "JoeLendingStrategyV3::failed to redeem");
            require(tokenDelegator.repayBorrow(unrollAmount) == 0, "JoeLendingStrategyV3::failed to repay borrow");
    ...
    function _stakeDepositTokens(uint256 amount) private {
        ...
        require(tokenDelegator.mint(amount) == 0, "JoeLendingStrategyV3::Deposit failed");
        uint256 borrowed = tokenDelegator.borrowBalanceCurrent(address(this));
    ...
    function _safeTransfer(
        ...
        require(IERC20(token).transfer(to, value), "JoeLendingStrategyV3::TRANSFER_FROM_FAILED");
    }
