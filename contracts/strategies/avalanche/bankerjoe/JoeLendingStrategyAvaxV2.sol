...
contract JoeLendingStrategyAvaxV2 is YakStrategyV2Payable, ReentrancyGuard {
    ...
    function _deposit(address account, uint256 amount) private onlyAllowedDeposits {
        require(DEPOSITS_ENABLED == true, "JoeLendingStrategyAvaxV2::_deposit");
        if (MAX_TOKENS_TO_DEPOSIT_WITHOUT_REINVEST > 0) {
            (uint256 joeRewards, , uint256 amountToReinvest) = _checkRewards();
            if (amountToReinvest > MAX_TOKENS_TO_DEPOSIT_WITHOUT_REINVEST) {
                _reinvest(joeRewards, amountToReinvest);
            }
        }
    ...
    function withdraw(uint256 amount) external override {
        ...
        require(depositTokenAmount > minMinting, "JoeLendingStrategyAvaxV2:: below minimum withdraw");
        _burn(msg.sender, amount);
        _withdrawDepositTokens(depositTokenAmount);
        (bool success, ) = msg.sender.call{value: depositTokenAmount}("");
        require(success, "JoeLendingStrategyAvaxV2::withdraw transfer failed");
    ...
    function _withdrawDepositTokens(uint256 amount) private {
        _unrollDebt(amount);
        require(tokenDelegator.redeemUnderlyingNative(amount) == 0, "JoeLendingStrategyAvaxV2::redeem failed");
        uint256 balance = tokenDelegator.balanceOfUnderlying(address(this));
        uint256 borrow = tokenDelegator.borrowBalanceCurrent(address(this));
        if (balance > 0) {
    ...
    function reinvest() external override onlyEOA nonReentrant {
        (uint256 joeRewards, , uint256 amountToReinvest) = _checkRewards();
        require(amountToReinvest >= MIN_TOKENS_TO_REINVEST, "JoeLendingStrategyAvaxV2::reinvest");
        _reinvest(joeRewards, amountToReinvest);
        claimAVAXRewards();
    }
     ...
     */
    function _reinvest(uint256 joeRewards, uint256 amount) private {
        if (joeRewards > 0) {
        ...
        uint256 fees = amount.mul(DEV_FEE_BIPS.add(REINVEST_REWARD_BIPS)).div(BIPS_DIVISOR);
    ...
    function _rollupDebt(uint256 principal, uint256 borrowed) internal {
        ...
        while (supplied < lendTarget) {
            ...
            require(tokenDelegator.borrowNative(toBorrowAmount) == 0, "JoeLendingStrategyAvaxV2::borrowing failed");
            require(
                tokenDelegator.mintNative{value: toBorrowAmount}() == 0,
                "JoeLendingStrategyAvaxV2::lending failed"
            );
    ...
    function _unrollDebt(uint256 amountToFreeUp) internal {
        ...
        while (toRepay > 0) {
            ...
            require(
                ...
                "JoeLendingStrategyAvaxV2::failed to redeem"
            );
            require(
                tokenDelegator.repayBorrowNative{value: unrollAmount}() == 0,
                "JoeLendingStrategyAvaxV2::failed to repay borrow"
            );
    ...
    function _stakeDepositTokens(uint256 amount) private {
        require(amount > 0, "JoeLendingStrategyAvaxV2::_stakeDepositTokens");
        require(tokenDelegator.mintNative{value: amount}() == 0, "JoeLendingStrategyAvaxV2::Deposit failed");
        uint256 borrowed = tokenDelegator.borrowBalanceCurrent(address(this));
    ...
    function _safeTransfer(
        ...
        require(IERC20(token).transfer(to, value), "JoeLendingStrategyAvaxV2::TRANSFER_FROM_FAILED");
    }
    ...
    {
        joeRewards = _getReward(0, address(this));
    ...
    function rescueDeployedFunds(uint256 minReturnAmountAccepted, bool disableDeposits) external override onlyOwner {
        ...
        require(
            ...
            "JoeLendingStrategyAvaxV2::rescueDeployedFunds"
        );
        emit Reinvest(totalDeposits(), totalSupply);
