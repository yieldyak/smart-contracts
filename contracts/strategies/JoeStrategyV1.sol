...
contract JoeStrategyV1 is YakStrategy {
    ...
    function _convertRewardIntoWAVAX(uint pendingJoe, address extraRewardToken, uint pendingExtraReward) private returns (uint) {
        ...
        if (pendingExtraReward > 0 && _checkSwapPairCompatibility(swapPairExtraToken, extraRewardToken, address(WAVAX))) {
    ...
    function _safeTransfer(address token, address to, uint256 value) private {
        require(IERC20(token).transfer(to, value), 'JoeStrategyV1::TRANSFER_FROM_FAILED');
    }
