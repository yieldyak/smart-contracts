...
contract OliveStrategyForSA is YakERC20, Ownable {
    ...
    uint256 private constant UINT_MAX = uint256(-1);
    ...
    constructor(
        ...
        name = string(abi.encodePacked("Yield Yak: ", depositToken.symbol()));
    ...
    function checkReward() public view returns (uint256) {
        uint256 pendingReward = stakingContract.pendingOlive(PID, address(this));
    ...
    function _reinvest(uint256 amount) internal {
        ...
        uint256 depositTokensToAdd = amount.sub(adminFee).sub(reinvestFee);
